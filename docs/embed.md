...
**Why Two Models?**
English documents get optimized processing with the specialized model, while multilingual content uses the broader model. This balances accuracy with processing efficiency.

### 2. Word2Vec Embeddings

**Global Training Strategy:**
The system trains a single Word2Vec model on ALL document content, then uses this global model to embed individual chunks.

**Benefits:**
- Consistent vocabulary across all documents
- Better handling of domain-specific terminology
- Improved semantic relationships within the climate domain
...
The system detects six types of logical relationships using pattern matching and semantic similarity:

1. **SUPPORTS** - Evidence supporting a claim
2. **EXPLAINS** - Detailed explanation of a concept
3. **CONTRADICTS** - Conflicting information
4. **FOLLOWS** - Sequential logical progression
5. **TEMPORAL_FOLLOWS** - Time-based sequence
6. **CAUSES** - Causal relationships
...
```sql
ALTER TABLE doc_chunks ADD COLUMN 
    transformer_embedding FLOAT[],   -- 768-dimensional BERT vectors
    word2vec_embedding FLOAT[],      -- 300-dimensional Word2Vec vectors
    chunk_data JSONB                 -- Metadata and processing info
```
...
CREATE TABLE logical_relationships (
    ...
    target_chunk_id UUID REFERENCES doc_chunks(id),
    relationship_type VARCHAR(50),
    confidence FLOAT CHECK (confidence BETWEEN 0.0 AND 1.0),
    evidence TEXT,
    method VARCHAR(50) DEFAULT 'pattern_matching'
);
```
