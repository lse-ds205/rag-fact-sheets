        ...
        try:
            if isinstance(answer, dict):
                ...
                if isinstance(summary, str) and summary.strip().startswith('{'):
                    try:
                        ...
                        if isinstance(parsed_summary, dict) and 'answer' in parsed_summary:
                            summary = parsed_summary['answer'].get(
                                'summary', summary
                            )
                ...
                if isinstance(detailed, str) and detailed.strip().startswith('{'):
                    try:
                        ...
                        if isinstance(parsed_detailed, dict) and 'answer' in parsed_detailed:
                            detailed = parsed_detailed['answer'].get(
                ...
                if (not summary or summary == '{}') and (not detailed or detailed == '{}'):
                    elements.append(Paragraph(
                        "Content not available in readable format.",
                        styles['DetailStyle']
                    ))
                    elements.append(Spacer(1, 12))
            else:
            ...
            elements.append(Paragraph(
                f"Error processing answer content: {str(e)}",
                styles['DetailStyle']
            ))
            elements.append(Spacer(1, 12))
...
def create_citations_section(response_data, styles):
    ...
    for country, country_citations in citations_by_country.items():
        ...
        for i, citation in enumerate(unique_citations, 1):
            ...
            citation_text = (
                f"[{i}] {content[:200]}{'...' if len(content) > 200 else ''}"
            )
            elements.append(Paragraph(citation_text, styles['CitationStyle']))
            
            if how_used:
                elements.append(Paragraph(
                    f"<i>Used for: {how_used}</i>", 
                    styles['CustomBody']
                ))
            
            if similarity:
                elements.append(Paragraph(
                    f"<i>Relevance Score: {similarity:.3f}</i>",
                    styles['CustomBody']
                ))
...
def main():
    ...
    try:
        ...
        for json_path in llm_files:
            try:
                logger.info(f"[6_OUTPUT] Processing: {json_path}")
                ...
                # Generate PDF
                logger.info(f"[6_OUTPUT] Generating PDF report for {country_name}...")
                generate_pdf_report(response_data, output_path, styles, country_name)
                
                logger.info(f"[6_OUTPUT] PDF report generated: {output_path}")
                logger.info(
                    f"[6_OUTPUT] Report contains analysis of "
                    f"{len(response_data.get('questions', {}))} questions for "
                    f"{country_name}"
                )
                
            except Exception as e:
                logger.error(f"[6_OUTPUT] Error processing {json_path}: {e}")
        
        return str(OUTPUT_DIR)
        
    except Exception as e:
        logger.error(f"[6_OUTPUT] Error in PDF generation process: {e}")
        return None
